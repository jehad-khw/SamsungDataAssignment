##check if "Samsung data" folder exists, if yes then choose it as workind directory, else stop
...
##(1) download the ZIP file to local workign directory, set the download date, and unzip it
...
##(2) For traing and test datasets,
...
##(3) merge both training and test data sets, then remove thier objects,
...
##(4) read features dataset to indetify mean and standard deviation for each measurement.
###### Request (2) Extracts only the measurements on the mean and ####################
...
##(5) read activities codes/labels to substitute codes with labels in Y
###### Request (3)  Uses descriptive activity names to name the ######################
...
##(6) Using names() function to change variable names for X, Y, subject datasets
###### Request (4) Appropriately labels the data set with .###########################
...
##(7) combine the three part of the dataset X,Y, and subject, then create a new tidy
## dataset in data.table formate to produce more calculation as requested
###### Request (5) From the data set in step 4, creates      #########################
